...
obs_diff <- mean(framework_cdm21$conception_duration) - mean(framework_cdm16$conception_duration)
R <- 5000
...
for (i in 1:R){
  permuted <- sample(combined_framework)
  
  perm_16 <- permuted[1:length(framework_cdm16$conception_duration)]
  perm_21 <- permuted[(length(framework_cdm16$conception_duration) + 1) : length(combined_framework)]
  
  perm_diff[i] <- mean(perm_21) - mean(perm_16)
}

p_value_framework <- mean(perm_diff >= obs_diff) # 2-sided permutation test

pl_framework <- ggplot() +
  geom_histogram(aes(x=perm_diff), bins=R, color="#2980B9",alpha=0.7) +
  ...
  labs(title="Permutation Test Sampling Distribution for Major Framework A",
       ...
       y="Frequency") +
  annotate("text", x = obs_diff - 20, y = 6, 
...
combined_framework <- c(open_cdm16$conception_duration,open_cdm21$conception_duration)
set.seed(32779267)
perm_diff <- numeric(R)
...
for (i in 1:R){
  ...
  perm_21 <- permuted[(length(open_cdm16$conception_duration) + 1) : length(combined_framework)]
  
  perm_diff[i] <- mean(perm_21) - mean(perm_16)
}

p_value_open <- mean(abs(perm_diff) >= abs(obs_diff)) # 2-sided permutation test
