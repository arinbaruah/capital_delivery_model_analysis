...
```{r}
dcmp <- project_tsibble |> filter(delivery_program == "Major Works - Small Scale" & year(`yearmonth(bca)`) <= 2024) |>
  model(stl = STL(Total ~ season(window = 11)))
```


```{r}
model_fit <- project_tsibble %>% filter(delivery_program == "Major Works - Small Scale" & year(`yearmonth(bca)`) <= 2024) |> model(
     Mean = MEAN(Total),
    Naive = NAIVE(Total),
...
model_fc <- model_fit |> forecast(h = 72,bootstrap =TRUE) %>%  mutate(Total = distributional::dist_truncated(Total, 0))

model_fc |> autoplot() +
   geom_line(
    data = project_tsibble %>% filter(delivery_program == "Major Works - Small Scale" & year(`yearmonth(bca)`) <= 2024),
...
```{r}
#| eval: FALSE
naive_sim <- project_tsibble %>% filter(delivery_program == "Major Works - Small Scale") |> model(
    Naive = NAIVE(Total))

sim <- naive_fit %>% select(Naive) |> generate(h = 6, times = 10, bootstrap = TRUE)
```


```{r}
#| eval: FALSE
project_tsibble %>% tail(52) |>
  ggplot(aes(x = `yearmonth(bca)`)) +
...
The selection of the ETS model would require us to study the timeseries data of the Major Works-Small Scale delivery program projects in detail and outline any important timeseries characteristics that might be beneficial to obtain the appropriate ETS model for forecasting the projects in the upcoming delivery model. 
...
```{r}
#| label: fig-decomp
#| fig-cap: "Time series decomposition for Major Works-Small Scale projects"

components(dcmp) %>% autoplot() + 
  labs(title = "Time series decomposition plot",
       subtitle = "Major Works-Small Scale",
       x = "Timeline",
...
ggplot(project_group_progam %>% filter(cdm != "CDM-26"),aes(x = Percentage,
  ...
  labs(title = "Distribution of delivery programs within each CDM",
       ...
       caption = "Melbourne Water Corporation",
...
@fig-compduration compares the duration of time taken to submit a business case for further approval once a project has been conceptualised. The comparison was done only for the delivery programs under Major Works-Framework and Major Works-Open Market as the other programs are generally fast-tracked by bypassing the PBC stage. Some of the key observations are as follows:
...
#| fig-cap: "Comparison of concept submission duration across CDMs"
#| fig-height: 4 
#| fig-width: 8
...
ggplot(data = project_data %>% filter(delivery_program %in% del_prg  & cdm != "CDM-26"),aes(y = day(conception_duration),x = delivery_program,fill = delivery_program)) + geom_boxplot()  + facet_wrap(~cdm) + scale_fill_tableau(palette = "Tableau 10") + 
  ...
  labs(x = "Delivery program",
       y = "Concept submission duration",
       caption = "Source: Melbourne Water Corporation", 
       title = "Distribution of duration for project concept submission",
       subtitle = "Number of days between BNI and PBC")
```
The other critical stage of the projects is to monitor the duration between the project approval (BCA) to project delivery (Stage 1). @fig-delduration and @fig-medduration illustrate how the duration of projects from approval to delivery stage differ across various delivery programs. Key insights from this visualisation are as follows:
...
2. The number of "Open Market" and "Contestable" projects are typically very low in number within any CDM. As a result, its distribution of completion duration is not expected to provide a representative figure for the future delivery model.
...
4. The distribution of completion durations for “Framework” and “Small-scale” projects indicates that the majority of these projects are completed in approximately one year. This insight is crucial for estimating the expected duration for a significant portion of projects in the upcoming CDM. It can help optimize resource and crew allocation, ensuring more efficient project management and planning.
...
ggplot(data = project_data %>% filter(status == "Completed"),aes(x = day(completion_duration),fill = delivery_program)) + geom_density()  + scale_fill_tableau(palette = "Tableau 10") + theme_minimal() + theme(legend.position = "none") + facet_wrap(~delivery_program,ncol = 1) +
  labs(x = "Completion duration (days)",
y = "Density",
caption = "Source: Melbourne Water Corporation",
...
```{r}
#| label: fig-medduration
#| fig-cap: "Median project completion duration across delivery programs"
#| fig-height: 3 
#| fig-width: 8

project_data %>% filter(status == "Completed") %>% 
  ...
  geom_label(aes(label = Median_completion)) +  theme_minimal() + theme(legend.position = "none") + 
  labs(x = "Duration (Days)",
       y = "Delivery program",
       title = "Median duration for project completion",
       subtitle = "Duration between BCA and Stage 1",
       caption = "Source: Melbourne Water Corporation")
```
