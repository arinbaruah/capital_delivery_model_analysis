...
#| fig-height: 4
#| fig-width: 8
model_fit %>% select(-c(ETS_ana,Naive,arima_auto)) %>% forecast(h = "2 years",bootstrap =TRUE) %>% mutate(Total = distributional::dist_truncated(Total, 0)) %>%
 autoplot() + geom_line(
...
#| fig-height: 4
#| fig-width: 8
model_fit %>% select(c(ETS_ana,arima_auto)) %>% forecast(h = "2 years",bootstrap =TRUE) %>%  mutate(Total = distributional::dist_truncated(Total, 0)) %>%
 autoplot() + geom_line(
    data = project_tsibble %>% filter(ffc_band == "< = 5" & `yearmonth(bca)` < ymd(20240601)),
    aes(x = `yearmonth(bca)`, y = Total, linetype = "Approved projects")) + guides(
    color = guide_legend(title = "Forecast model"),
...
#| fig-height: 4
#| fig-width: 8
model_fc <- model_fit %>% select(ETS_ana,arima_auto) |> forecast(h = 72,bootstrap =TRUE) %>%  mutate(Total = distributional::dist_truncated(Total, 0))
