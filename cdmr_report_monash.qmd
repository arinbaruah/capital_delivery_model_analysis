...
ggplot(project_group_progam %>% filter(cdm != "CDM-26"),aes(x = Percentage,
  ...
  labs(x = "Percentage of FFC",y = "Capital Delivery Model",
       title = "Distribution of total valuation of projects across CDMs and FFC bands",
       caption = "Source: Melbourne Water Corporation",
       fill = "FFC (in mil $)") +
  geom_label_repel(aes(label = scales::percent(Percentage, accuracy = 1)), 
            position = position_fill(vjust = 0.5), size = 3,color = "white",fontface = "bold") +
...
2. Projects with higher valuations and risk are approved by higher authorities who maybe external to the department or the organisation. In such cases, approval duration is expected to be considerably higher, with some projects taking more than an entire year to reach the "approved" stage. The __current study recommends breaking down projects of large valuations into smaller "bundled" projects that allows for a faster approval process within the same department or the organisation.__
...
3.	__Advanced Forecasting with PowerBI:__ While PowerBI has the ability to provide under-the-hood forecasts, however to replicate the specialised forecast methods from the “Fable” package, PowerBI has the ability to run R scripts within the dashboard, thereby allowing us to tweak the forecast models to learn better from the historical data and improve the accuracy of the future predictions, as well as allow users to view these insights through a user-friendly PowerBI dashboard.

4. __Visualisation and Accessibility:__ To make the current study available to a broader audience, the insights and visualisations can be replicated on a PowerBI organisational dashboard. This would have the added benefit of leveraging Microsoft Fabric’s datalake where the live data can be extracted, transformed and loaded onto the dashboard, providing insights based on live project data.

5. __Implementing KRA and KPI dashboard reporting:__ Incorporating Key Result Areas (KRAs) and Key Performance Indicators (KPIs) into the dashboard facilitates effective tracking of current projects against historical data and organizational benchmarks. These indicators enable organsations to assess project delivery performance and efficiency across different service providers and project types. By developing these metrics in close consultation with business leaders, they can be tailored to align with strategic objectives and then visualized for quick and intuitive access on the dashboard.
