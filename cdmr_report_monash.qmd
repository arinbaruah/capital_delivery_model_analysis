...
tibble(
  ...
  "Approval authority" = c("Department of Treasury and Finance"," Melbourne Water board","Finance, audit and compliance","Managing director","Executive leadership group","Senior leadership group","People leadership group")
) %>%
  kable("latex") %>%
  kable_styling(latex_options = c("striped"), 
                full_width = FALSE, 
...
framework_cdm16 <- project_data %>% filter(cdm == "CDM-16" & 
                                             delivery_program == "Major Works - Framework") %>% 
...
open_cdm16 <- project_data %>% filter(cdm == "CDM-16" & 
                                             delivery_program == "Major Works - Open Market") %>% 
  select(conception_duration) %>% mutate_all(day) %>% drop_na()
