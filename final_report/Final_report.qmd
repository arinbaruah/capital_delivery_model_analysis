...
3. __BCA (Business Case Approval)__ : A date on which the business case is approved and the project  
...
14. __KRA (Key results area)__: A set of collective metrics which provide the current progress or performance in a broad area of interest.
15. __KPI (Key performance indicator)__: A single specific metric which provides the current progress or performance in a focused area of interest.

# GitHub link

All the files required to reproduce this analysis can be referred to on this [GitHub repository](https://github.com/arinbaruah/capital_delivery_model_analysis).
...
4.	__Approval Duration Distribution:__ Assessment of the time required for project approvals across various delivery programs and critical project stages. 
...
```{r read-data}
# Reading data and using de-identified delivery program names

project_data <- read_csv("../data/Capital_Delivery_Project_deidentified.csv")
project_data <- clean_names(project_data)
project_data <- project_data %>% mutate(delivery_program = case_match(delivery_program,"Major Works - Small Scale" ~ "Minor Framework A",
                                                                      "Contestable" ~ "Minor Framework B" ,
                                                                      "Major Works - Framework" ~ "Major Framework A",
                                                                      "Major Works - Open Market" ~ "Major Framework B"
                                                                      ))
```
...
# Additive timeseries decomposition plots

dcmp <- project_tsibble |> filter(ffc_band == "< = 5" & `yearmonth(bca)` < ymd(20240601)) |>
  model(stl = STL(Total ~ season(window = 13),robust = TRUE)) 
...
Additionally, a separate dataset was used to implement a KRA-KPI methodology for tracking project progress and incentivising improved performance among service providers. This dataset, submitted as a Microsoft Excel sheet, was directly populated by the service providers at the end of each month. 
...
4. The KRA-KPI tracking dataset is currently in a Microsoft Excel format where the information is not presented in a tidy, structured manner, making it unsuitable for analysis in R or PowerBI. Manually cleaning and reformatting the data into a tidy format could introduce potential inaccuracies, which would require additional filtering at a later stage.

5. The dataset here is an observational data. Some of the limitations that are prevalent in such datasets are as follows :
...
The construction of large-scale infrastructure projects, commonly referred to as “CAPEX” projects, is highly resource-intensive and typically requires effective collaboration among multiple organisations for successful and timely completion. @no2013specification suggests that in a collaborative work environment, teams are required to generate information using standardised procedures and agreed-upon standards and methods. This ensures consistency in form and quality, allowing the information to be used and reused without the need for modification or interpretation. However, in practice, such high level collaboration is often challenging and needs well laid communication as well as contractual frameworks for effective project delivery.
...
@garcia2021measuring states that a key underlying challenge in today’s model-driven approach to engineering is how progress and level of effort are being measured and reported. The goal of the current project is to quantify the key results of the past delivery models and obtain important insights which would allow for to design an improved capital delivery model for the upcoming pricing submission period between 2026-2031. The analysis aims to forecast the future number of projects, estimate their expected valuations based on project distribution patterns, and develop key performance metrics. These performance metrics will offer a comprehensive view of project progress while enabling Melbourne Water to effectively incentivise service providers to meet contractual goals, ensuring both efficiency and accountability in project delivery.

# Motivation of the current study

1. The process of estimating the mix of the type of projects expected in the new capital delivery model after the 2026 price submission is based on the analysis of the current delivery model (CDM-21) and the previous delivery model (CDM-16). The forthcoming 2026 price submission is projected to surpass recent submissions in value, presenting significant challenges in project management. This includes the engagement of service providers for construction, efficient management of key crew resources, and rigorous monitoring to ensure timely infrastructure delivery. The current data analysis review of the projects in CDM-16 and CDM-21 will provide an adequate baseline of the expected proportion of the major capital delivery projects based on valuation and duration expected in the future delivery model (CDM-26), thereby providing insights and eventually aiding in the key decision making for creation of the frameworks.

2. The KRA-KPI tracker templates and dashboards will enable Melbourne Water to conduct fair and transparent assessments of service providers assigned to projects in an efficient and quick manner. They will further highlight any calculation discrepancies and flag historical data changes, while the dashboard will provide a comprehensive overview of assessments. This approach adds a layer of verification, ensuring data integrity and facilitating quicker decision-making and assessment turnarounds.
...
The objectives and the scope of the analysis of the CDMs were defined through Melbourne Water fortnightly workshops and feedback as received from the project director and the steering group. These are delineated as follows:
  ...
  1. The analysis must clearly indicate the performance level of project deliveries for both the current CDM-21 and the previous CDM-16. This should primarily focus on the duration between critical project stages and resource allocation, highlighting any notable changes in this aspect.
  ...
  4. Adoption of templates and dashboards that allow Melbourne Water to track and verify the current project progress and further facilitates fair and transparent assessment of the service providers during the course of project execution or delivery.
  
# Significance of the current study

Based on the numerous inputs from the project director and the steering group at Major Capital Delivery division in Melbourne Water, the scope and significance for the current CDM data analysis project was finalised. These have been delineated as follows:  
  ...
  8.  The implementation of the KRA-KPI tracking templates is expected to identify potential discrepancies in service providers’ project progress calculations, enabling faster turnaround of assessment reports. Additionally, these templates offer the benefit of reusability for future projects, establishing a consistent structure for tracking and assessing service provider performance.
...
While the dataset contains the details for projects undertaken by the various divisions of Melbourne Water Corporation, for the purpose of the current analysis, only those projects which are delivered or expected to be delivered by the "Major Capital Delivery" division are considered. In particular, these are those projects which are delivered through a contractual framework (Major Framework A,Minor Framework B), engineering maintenance projects to keep current assets running (Minor Framework A), or in certain instances, large projects which are tendered in the open market (Major Framework B).

In the current dataset, these projects are those which come under the delivery programs as tabulated in @tbl-programs. All other delivery programs have been filtered out of the dataset.

```{r}
#| label: tbl-programs
#| tbl-cap: "Delivery programs under Major Capital Delivery division"
#| tbl-pos: H
tibble(
  "Capital Delivery Program" = c("Minor Framework A", "Major Framework A",
                                        "Minor Framework B", "Major Framework B")
) %>%
  kable("latex") %>%
...
Each capital project which is to be delivered within a CDM will require appropriate approvals from the relevant authority. The exact level of authorisation is generally based on the contract value of the project as well as the risk associated with each project. As the contract value is not readily available, the FFC value is used instead to simplify the analysis. As a result, the total percentage of projects attributed to each authority is a conservative estimate of the actual expected projects since the FFC valuations are higher than contract values. The FFC valuations that govern the authorisation level for a project is delineated in @tbl-approval.
...
tibble(
  "FFC of project ($)" = c("> 125 mil","75-125 mil","40-75 mil","8-40 mil","5-8 mil","2.5-5 mil","0-2.5 mil"),
  "Approval authority" = c("Government approval"," Board of directors","Finance and audit","Managing director","Level 1 authority","Level 2 authority","Level 3 authority")
) %>%
  kable("latex") %>%
  kable_styling(latex_options = c("striped"), 
                full_width = FALSE, 
...
The current analysis attempts to forecast the number of projects that are expected in CDM-26. For this purpose, the project data corresponding to the delivery models CDM-16 and CDM-21 are utilised to forecast the numbers in the future delivery model. As explained in @sec-limit, due to data migration and data integrity issues for the projects in the CDM-11 delivery model, this is not utilised as part of the current analysis.

Additionally, the forecasts were calculated for the projects with FFC valuations of 5 mil $ or lower. As explained in @sec-prodist, these subset of projects tend to form the majority of the projects for any delivery model and also requires about as much labour hours as all other projects combined. Hence, a forecasted number of projects in each month and year of the new delivery model can allow for better crew resource management and will additionally aid in proactive procurement of raw materials rather than reactive procurement, allowing for more efficient delivery process.
...
tibble(
  "Forecast method" = c("Mean", "Croston","Seasonal Naïve","ETS")) %>%
  kable("latex") %>%
  kable_styling(latex_options = c("striped"), 
                full_width = FALSE, 
                position = "center") %>%
...
Analysing the decomposed data enables us to select the most appropriate type of ETS model for forecasting the number of projects in the upcoming delivery period. An additive decomposition of a timeseries data is obtained based on @eq-decomp. 

$$ \boxed{y_t = S_t + T_t + R_t}$$ {#eq-decomp}

Where,

$y_t$ is the data of interest, $S_t$ is the seasonal component, $T_t$ is the trend component and $R_t$ is the remainder.
...
1. There appears to be a stationary trend in the data other than the recent months. This indicates that there is no clear additive or multiplicative trend in the data which can be modeled through the ETS model.

2. The seasonality of the current timeseries data indicates a peak close to the middle of the year for approved projects. Additionally, the seasonality appears to be fairly consistent throughout the years, indicating additive seasonality of the data.

3. The scale of the remainder appears to be large, meaning that the model is unable to capture some important characteristics in the timeseries, possibly due to the limited availability of the current data and a few large outliers.
...
# Plotting the additive decomposition of timeseries data

pl_decomp <- components(dcmp) %>% autoplot() + 
...
ggsave("../images/stl_decomposition.jpeg",pl_decomp,width = 8,height = 4)
```
...
resids <- gg_tsresiduals(dcmp) + labs(x = "Timeline",
                            title = "Residual diagnostics for timeseries data",
                            subtitle = "Projects with FFC <= 5 mil $") 
resids
ggsave("../images/residuals.jpeg",resids,width = 8,height = 4)
```
\begin{warningBox}
The current forecast models face limitations due to the unavailability of project data beyond the current and past CDM. This resulted in forecasts exhibiting high uncertainty, characterised by large residuals and broad prediction intervals.
...
![KRA-KPI flowchart](../images/kra_kpi.png){#fig-flowchartkpi fig-height=3}
...
1. In each of the CDMs, the "Major Framework B" projects constitute the smallest proportion. This is usually the case because such projects tend to incur a large sum of FFC and some of these projects could also be "green-field" projects that take a long time period for delivery.

2. While the percentage of "Major Framework A" projects reduced from CDM-16 to CDM-21, a subsequent rise in "Minor Framework A" and "Minor Framework B" projects were observed within the same period. It is especially interesting to note that the current model delivers nearly half of all the projects through the "Minor Framework A" program.

3. Allocating more projects through the delivery programs of "Minor Framework A" and "Minor Framework B" route in the current model was expected to expedite the delivery period of projects as these programs typically require fewer approvals and consequently, lesser time to reach the BCA stage.
...
cc <- c("Minor Framework B" = "#FF6F61" , 
        "Major Framework A" = "#40E0D0" ,
        "Major Framework B" = "#FFD700",
        "Minor Framework A" = "#87CEFA")

pl1 <- ggplot(project_group_progam %>% filter(cdm != "CDM-26"),aes(x = Percentage,
                                y = cdm,
...
pl1
ggsave("../images/distribution_delivery_programs.jpeg",pl1,width = 8,height = 4)
```
## Distribution of projects by FFC bands for each delivery model {#sec-prodist}

@fig-ffcband and @fig-ffcprop illustrate the distribution of the project FFCs associated with each CDM. Key observations are delineated below:
...
2. Though most projects are individually valued at $5 million or less, these lower-valued projects make up only 8% of the total spend in CDM-21.

3. On the other hand, projects which are worth 50 mil $ or higher remain to be the smallest category of projects in each of the CDMs.

4. Analysis of the current and the previous CDMs suggest that the new delivery model (CDM-26) would be expected to constitute majority of the projects with FFCs below 5 mil $.

5. Such projects are associated with works such as laying new pipelines, pipeline renewals, mechanical or electrical machinery maintainence and other works of similar nature. As a result, the demand for these projects in each year are expected to remain fairly consistent, and hence provides valuable insights on the future CDM.
...
pl2 <- ggplot(project_group_progam %>% filter(cdm != "CDM-26"),aes(x = Percentage,
                                y = cdm,
...
pl2
ggsave("../images/distribution_cost_bands.jpeg",pl2,width = 8,height = 4)
```
...
pl3 <- ggplot(project_group_progam %>% filter(cdm != "CDM-26"),aes(x = Percentage,
                                y = cdm,
                                fill = ffc_band)) + geom_col(position = "fill") +
...
pl3
ggsave("../images/small_scale_cost_distribution.jpeg",pl3,width = 8,height = 4)
```
```{r}
#| eval: false
#| fig-cap: "Cumulative project valuations across different CDMs and FFC bands"
...
project_group_progam <- project_data %>% 
  mutate(program = case_match(ffc_band,"< = 5" ~ "< = 5",
                              .default = "All other projects"))

project_group_progam <- project_group_progam %>%
  group_by(program, cdm) %>%
  summarise(Total = sum(ffc)) %>%
  ungroup() %>%
  group_by(cdm) %>%
  mutate(Percentage = Total / sum(Total))
...
color_codes <- c("All other projects" = "gray","< = 5" = "#D35400")

pl <- ggplot(project_group_progam %>% filter(cdm != "CDM-26"),aes(x = Percentage,
                                y = cdm,
                                fill = program)) + geom_col(position = "fill") +
  labs(x = "Percentage of projects by total valuation",y = "Capital Delivery Model",
       title = "Distribution of total valuation of projects across delivery models and cost bands",
       fill = "Cost bands (in mil $)") +
    geom_label_repel(aes(label = scales::percent(Percentage, accuracy = 1)), 
...
ggsave("../images/small_scale_cost_distribution.jpeg",pl,width = 8,height = 4)
...
1. More than half of the projects within a CDM are expected to be approved by the "Level 3 authority". Based on the past approval durations, these projects are those with smaller FFC valuations and may expect quicker approval. In the future delivery model, this trend is expected to continue. For projects which may exceed the approval limit and eventually take longer time to get approved, they can be further broken down into smaller projects of lower FFC valuations and bundled together to benefit from the quicker approval periods.

2. The percentage of projects approved by the “Level 2 authority”, “Finance and Audit” and “Managing Director” has remained fairly consistent over the past two CDMs, suggesting a stable pattern in the composition of future approvals. Analyzing the approval durations, as shown in @fig-approvaltime, offers a benchmark for the time required in the approval process. This information can be used to plan effectively and address potential risks associated with approval delays in advance.

3. With an anticipated increase in the number of projects which will require approval from the Government (Department of Treasury and Finance) in the upcoming delivery program, it is important to address potential delays. Historically, these projects face prolonged approval times due to associated costs, risks, and the need for coordination with external government agencies. To streamline the approval process, it is advisable to negotiate special considerations or exemptions with the ministry and DTF. This approach could help mitigate barriers and facilitate a more timely approval for these complex, large-scale, high-risk projects.
...
# Using de-identified approval group names
...
# Using ascending order of authorities 
...
ggsave("../images/approval_duration_distribution.jpeg",pl4,width = 8,height = 4)
```
...
dim_order <- c("Government approval","Board of directors","Finance and Audit","Managing Director",
               "Level 1 authority","Level 2 authority","Level 3 authority")
               

project_group_approval$approval_body_dummy <- factor(project_group_approval$approval_body_dummy, levels = dim_order)

pl5 <- ggplot(project_group_approval %>% filter(cdm != "CDM-26"),aes(x = Percentage,
                                y = cdm,
...
pl5
ggsave("../images/approval_bodies_duration.jpeg",pl5,width = 8,height = 4)
```

## Labour utilisation analysis for each delivery program

@fig-labour depicts the distribution of the labour hours across each of the four delivery programs within MCD. The plot indicates that the current model is capable of delivering multiple small-scale and low risk contestable projects. The labour hours associated with Major Framework A and Minor Framework A appear to be fairly similar, despite the fact that the __Small Scale projects account for about only 10% of the FFC in a delivery period__. The future CDM for PS-26 should consider __streamlining or bundling multiple small-scale projects together to generate higher efficiency in Project Management labour resource allocation.__ 
...
del_prg <- c("Major Framework A","Major Framework B")

framework_cdm16 <- project_data %>% filter(cdm == "CDM-16" & 
                                             delivery_program == "Major Framework A") %>% 
  select(conception_duration) %>% mutate_all(day) %>% drop_na()
framework_cdm21 <- project_data %>% filter(cdm == "CDM-21" & 
                                             delivery_program == "Major Framework A") %>% 
  select(conception_duration) %>% mutate_all(day) %>% drop_na()

open_cdm16 <- project_data %>% filter(cdm == "CDM-16" & 
                                             delivery_program == "Major Framework B") %>% 
  select(conception_duration) %>% mutate_all(day) %>% drop_na()
open_cdm21 <- project_data %>% filter(cdm == "CDM-21" & 
                                             delivery_program == "Major Framework B") %>% 
  select(conception_duration) %>% mutate_all(day) %>% drop_na()

# Permutation test for Major Framework A projects

obs_diff_framework <- mean(framework_cdm21$conception_duration) - mean(framework_cdm16$conception_duration)
R <- 5000

combined_framework <- c(framework_cdm16$conception_duration,framework_cdm21$conception_duration)
set.seed(32779267)
perm_diff_framework <- numeric(R)

for (i in 1:R){
  permuted_framework <- sample(combined_framework)
  
  perm_16_framework <- permuted_framework[1:length(framework_cdm16$conception_duration)]
  perm_21_framework <- permuted_framework[(length(framework_cdm16$conception_duration) + 1) : length(combined_framework)]
  
  perm_diff_framework[i] <- mean(perm_21_framework) - mean(perm_16_framework)
}

p_value_framework <- mean(perm_diff_framework >= obs_diff_framework) # 1-sided permutation test

pl_framework <- ggplot() +
  geom_histogram(aes(x=perm_diff_framework), bins=R, color="#2980B9",alpha=0.7) +
  geom_vline(aes(xintercept=obs_diff_framework), color="red", linetype="dashed") +
  labs(title="Permutation Test Sampling Distribution for Major Framework A",
       x="Difference in mean PBC duration (CDM 21 - CDM 16)",
       y="Frequency") +
  annotate("text", x = obs_diff_framework - 140, y = 6, 
           label = paste("Observed\nDifference =", 
                         round(obs_diff_framework, 0)," days"), color = "red") +
  theme_minimal()

ggsave("../images/permutation_test_framework.jpeg",pl_framework,width = 8,height = 4)

```

```{r}
# Permutation test for Major Framework B projects

obs_diff <- mean(open_cdm21$conception_duration) - mean(open_cdm16$conception_duration)
R <- 5000

combined_open <- c(open_cdm16$conception_duration,open_cdm21$conception_duration)
set.seed(32779267)
perm_diff <- numeric(R)

for (i in 1:R){
  permuted <- sample(combined_open)
  
  perm_16 <- permuted[1:length(open_cdm16$conception_duration)]
  perm_21 <- permuted[(length(open_cdm16$conception_duration) + 1) : length(combined_open)]
  
  perm_diff[i] <- mean(perm_21) - mean(perm_16)
}

p_value_open <- mean(perm_diff >= obs_diff) # 1-sided permutation test


pl_open <- ggplot() +
  geom_histogram(aes(x=perm_diff), bins=R, color="#F5B041", alpha=0.7) +
  geom_vline(aes(xintercept=obs_diff), color="red", linetype="dashed") +
  labs(title="Permutation Test Sampling Distribution for Major Framework B",
       x="Difference in mean PBC duration (CDM 21 - CDM 16)",
       y="Frequency") +
  annotate("text", x = obs_diff + 260, y = 7, 
           label = paste("Observed\nDifference =", 
                         round(obs_diff, 0)," days"), color = "red") +
    theme_minimal()


ggsave("../images/permutation_test_open.jpeg",pl_open,width = 8,height = 4)
...
@fig-compduration compares the duration of time taken to submit a business case for further approval once a project has been conceptualised. This is quantified by calculating the duration between the BNI stage and the PBC stage of the project lifecycle. The comparison was done only for the delivery programs under Major Framework A and Major Framework B as the other programs are generally fast-tracked by bypassing the PBC stage. Some of the key observations are as follows:

1. In each of the CDMs, the duration of time for project conceptualisation is higher for Major Framework B as compared to Major Framework A. This is typically the case due to the fact that a business case for Major Framework A projects are contracted to a list of already approved service providers and will be engaged through a specific working contract which was agreed between the parties previously. On the other hand, Major Framework B projects are those that need to be tendered in the market and may require to form a working partnership with a new service provider. This leads to the requirement of further due-diligence, risk and cost analysis, contractual deliverables review and liaising with other government bodies to finalise a business case. As a result, the median duration for project concept submission is typically longer for these projects.

2. The median and mean time taken between the BNI and PBC stages, referred to as the project conceptualisation stage, has increased from CDM-16 to CDM-21. In CDM-21, the submission duration for each delivery program exceeded 2 years, which is longer than the durations observed in CDM-16. For future CDMs, it is advisable to consider these extended durations and explore strategies to reduce the time required for projects to progress from BNI to PBC, aiming to align with the shorter durations seen in CDM-16. 

3. To analyse if the increase in duration from CDM-16 to CDM-21 was statistically significant for each of the delivery programs, a one-sided hypothesis test based on the non-parametric permutation test was performed by considering a 95 % confidence interval. These hypotheses are delineated through @eq-null and @eq-alternate. The results of this test as illustrated through @fig-permutation suggested that there was indeed a __significant rise in the mean PBC duration for "Major Framework A" from CDM-16 to CDM-21__ (P-value: `r round(t.test(framework_cdm21,framework_cdm16)$p.value,3)`) while the same was not true for "Major Framework B" (P-value: `r round(t.test(open_cdm21,open_cdm16)$p.value,3)`).

4. Several outliers were observed in the business case submission duration for Major Works-Framework projects under CDM-16. These outliers were significantly higher than the median durations for both Framework A and Framework B. Investigating the causes of these delays can provide valuable insights. Understanding these issues is expected to benefit the upcoming CDM by helping to streamline the business case submission process and minimize any inadvertent delays.

5. These outliers suggest that the assumption of a normal distribution is likely violated, making a parametric approach such as T-test unsuitable for this dataset. Hence, a non-parametric test which is robust to presence of outliers and does not make an assumption of the distribution of the data was implemented. 

$$H_o : \mu_{\text{CDM-21}} <= \mu_{\text{CDM-16}}$$ {#eq-null}
$$H_1: \mu_{\text{CDM-21}} > \mu_{\text{CDM-16}}$$ {#eq-alternate}

Where $\mu$ is the average duration (in days) to submit a business case.
```{r}
...
pl6 <- ggplot(data = project_data %>% filter(delivery_program %in% del_prg  & cdm != "CDM-26"),aes(y = day(conception_duration),
                                                                                            x = delivery_program,
                                                                                            fill = delivery_program)) + geom_boxplot(aes(color = "Median")) + 
  stat_summary(fun = mean, geom = "pointrange", aes(color= "Mean"), 
               fatten = 2) +
...
ggsave("../images/pbc_duration.jpeg",pl6,width = 8,height = 4)


#t.test(open_cdm21,open_cdm16)$p.value 
```

```{r}
#| label: fig-permutation
#| fig-cap: "Permutation test sampling results for Major Framework A and B"
#| fig-height: 4 
#| fig-width: 8
#| fig-pos: "H"

pl_permutations <- pl_framework / pl_open
pl_permutations

ggsave("../images/permutation_test.jpeg",pl_permutations,width = 8,height = 4)
```
...
2. The number of Major Framework B and Minor Framework B projects are typically very low in number within any CDM. As a result, its distribution of completion duration is not expected to provide a representative figure for the future delivery model due to the resultant spread of the data.

3. On the other hand, both the Minor Framework A and Major Framework A projects together constitute more than 2/3rd of the total proportion of projects. As a result, the distribution of the project completion duration may provide a good prior information for the upcoming delivery model under vastly similar circumstances.

4. The distribution of completion durations for Major Framework A and Minor Framework A projects indicates that the majority of these projects are completed in __approximately one year.__ This insight is crucial for estimating the expected duration for a significant portion of projects in the upcoming CDM. It can help optimise resource and crew allocation, ensuring more efficient project management and planning.

5. The completion duration for projects under Minor Framework B and Major Framework B projects are comparable to one another and spans for __nearly a year and three months.__ 

```{r}
project_data <- project_data %>% mutate(completion_duration = days(stage1 - bca)) 

project_data <- project_data %>%
  mutate(delivery_program = factor(delivery_program, levels = c("Minor Framework A", "Minor Framework B", "Major Framework A", "Major Framework B")))
...
y = "Density",
title = "Distribution of project completion duration",
...
medduration <- medduration %>% 
  ggplot(aes(y = fct_reorder(delivery_program,-Median_completion), x = Median_completion, fill = delivery_program)) + 
...
ggsave("../images/completion_duration.jpeg",pl7,width = 8,height = 4)
```
...
3. The current models encounter a significant challenge when forecasting small discrete values, such as the number of expected monthly projects. As noted by @hyndman2018forecasting, these statistical models produce continuous value forecasts and can contain negative lower prediction intervals when the mean is close to 0. Such an outcome is not meaningful for expected number of projects as forecasts. In this analysis, a specialised implementation of the ETS model, known as Croston’s method, is attempted and compared against other benchmark and ETS models. While Croston’s method is better suited for handling data with zeros or small counts, it has a key limitation as per @shenstone2005stochastic : it does not correspond to a formal statistical model, and thus, prediction intervals cannot be computed. After weighing all the limitations of the models, it was decided that the best performing ETS model with an informative seasonality component will be used for final forecasts. In the current analysis, this model is ETS (A,N,A) and is further delineated in later sections.  
...
# Fit forecast models on training data
...
#| fig-width: 8
#| fig-pos: "H"
pl8 <- model_fit %>% select(-c(ETS_ana,Croston)) %>% forecast(h = "2 years",bootstrap =TRUE) %>% mutate(Total = distributional::dist_truncated(Total, 0)) %>%
 autoplot() + geom_line(
  ...
  labs(title = "Benchmark forecast model performance in test dataset",
       ...
       x = "Timeline") + theme_minimal()
pl8
ggsave("../images/benchmark_forecasts.jpeg",pl8,width = 8,height = 4)
```

@sec-etsmethod delineated the process of selecting the ETS method which will be used for forecasting the number of projects. Based on the decomposition of the timeseries data, the type of ETS model which will be used for forecasting is the __ETS (A,N,A) model__ and is further explained through @eq-forecast. 


$$\boxed{y_t = l_{t-1} + s_{t-m} + \epsilon_t}$$ {#eq-forecast}
Where, \
$y_t$ is the field of interest. \
$l_{t-1}$ is the level of the timeseries in one previous time step. \
$s_{t-m}$ is the seasonality in one seasonal time step behind. \
and $\epsilon_t$ is the residual error of the forecasts. \
The level and seasonality of the timeseries are further calculated using @eq-level and @eq-season.

$$l_t = l_{t-1} + \alpha \epsilon_t $$ {#eq-level}
$$s_t = s_{t-m} + \gamma\epsilon_t $$ {#eq-season}
Where $\alpha$ and $\gamma$ are hyperparameters and are __optimised by minimising the sum of squared errors (SSE).__


@fig-forecompvizets illustrates the performance of the model on the test dataset. Some of the key observations are as follows:
...
2. Current forecast model is limited by the relatively short duration of projects in the training dataset. In addition to this, the weak seasonality pattern in the data may have further added uncertainties in the forecast. As a result, the prediction intervals are moderately wide. While forecasting for future delivery models, as the size of the training dataset enlarges, the forecast models would be expected to be more confident and have narrower prediction intervals.
...
4. The actual approved projects consistently fell within the 80% and 95% prediction intervals for nearly the entire duration of the test dataset. This suggests that the current ETS model has effectively captured the information within the level of permissible uncertainties.
...
# Run forecast computation on test dataset and overlay the actual data

pl9<- model_fit %>% select(ETS_ana) %>% forecast(h = "2 years",bootstrap =TRUE) %>%  mutate(Total = distributional::dist_truncated(Total, 0)) %>% 
 autoplot() +  geom_line(
    data = project_tsibble %>% filter(ffc_band == "< = 5" & `yearmonth(bca)` < ymd(20240601)),
    aes(x = `yearmonth(bca)`, y = Total, linetype = "Approved projects")) + guides(
    color = guide_legend(title = "Forecast model"),
  ...
  theme_minimal()
pl9
ggsave("../images/ets_ana_model_forecast_test.jpeg",pl9,width = 8,height = 4)
```
...
pl10 <- model_fc |> autoplot() +
  ...
  labs(
    ...
    title = "Forecasted number of projects in CDM-26 using ETS (A,N,A) model",
    subtitle = "Projects with FFC <= 5 mil $"
  ) + theme_minimal()

pl10
ggsave("../images/ets_ana_model_forecast_results.jpeg",pl10,width = 8,height = 4)
```
## Tracking service provider’s performance through KPIs and KRAs

Based on the broad critical areas of assessments as illustrated by @fig-flowchartkpi, a total of 14 key performance indicators (KPIs) were identified, each catering to a specific key result area (KRA). The calculations of these metrics have the following requirements.

- Each KPI must be a fair assessment of the service provider's progress in the project.
- KPI calculations performed by the service provider must accurately demonstrate the various initiatives taken by them to meet project requirements.
- All calculations must be reproducible and be supported by supplementary data.
- The KPI calculations will be further evaluated by Melbourne Water, with a focus on identifying and addressing any discrepancies during the reproduction process.
- Must incentivise the correct behaviour in the service provider

In order to summarise the KRA-KPI  assessment results for a particular project, a dashboard on Microsoft PowerBI is generated, as illustrated by @fig-dashboard. The aim of this dashboard is to understand the current KPI scores provided by the service provider and what is its variation when compared to the assessments performed by Melbourne Water.

In addition to the KRA tracker dashboard, a structured Microsoft Excel template, as shown in @fig-template, was developed to perform scenario-based weighted scoring of KPIs. This template linked performance outcomes to financial payouts for the service provider. By doing so, Melbourne Water was able to __incentivise initiatives that were timely, safe, inclusive, and sustainable.__ Simultaneously, the service provider could assess the return on investment for their current initiatives and project potential payouts by improving their performance.


![Project KRA tracker dashboard](../images/dashboard.png){#fig-dashboard}

![Scenario based performance payout template](../images/payout.png){#fig-template}
...
1. Based on the analysis of sample data from CDM-21 and CDM-16, the new delivery model is projected to successfully deliver __approximately 70% of the projects with a forecasted final cost (FFC) of $5 million or less.__
...
2. __Incorporating Inflation Factors in project valuation forecasts:__ While current forecasting methods are capable of predicting the total valuation of projects, they do not account for inflationary factors such as increases in raw material and manpower costs. Although forecasting the project valuations for CDM-26 was not under the scope of the current project, future developments in forecasting methods should incorporate the Consumer Price Index (CPI) to better estimate the total expected valuation of projects, reflecting the impact of inflation on project costs. Such forecasts would allow the project teams to get an informed idea regarding the various cost allocations.
...
10. __ChatGPT__: OpenAI. (2024). ChatGPT [Large language model]. https://chat.openai.com
